...
so the sandbox mask keeps the narrow list it needs — with `.kojo/artifacts/` and `.kojo/data/`
granted ahead of everything, because every stamped factory ships `alwaysWritable: []` and an agent
that cannot record its work is one whose failure nobody can read. Three projects in the `tsc` build.
