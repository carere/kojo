 ...
 */
const started = (execution: Effect.Effect<string, never, unknown>) =>
  Effect.gen(function* () {
    const executionId = yield* execution as Effect.Effect<
      string,
      never,
...
const statusOf = (executionId: string) =>
  Effect.map(serviceFreeWorkflowEffect(lane.definition.poll(executionId)), (polled) =>
    Option.match(polled, {
  ...
  it.effect("gives the region the workspace its own sandbox chose", () =>
    Effect.gen(function* () {
      ...
      const result = Option.getOrThrow(
        yield* serviceFreeWorkflowEffect(lane.definition.poll(executionId)),
      );
  ...
  it.effect("stamps the run, the acquisition and the attempt on every sandbox", () =>
    Effect.gen(function* () {
      ...
      expect(Object.keys(environment ?? {}).sort()).toEqual([
        "KOJO_ATTEMPT",
        "KOJO_PHASE_ID",
        "KOJO_RESOURCE_ACQUISITION_KEY",
        "KOJO_RESOURCE_INSPECTION_FILE",
        "KOJO_RESOURCE_PROVIDER_IDENTITY",
        "KOJO_RUN_ID",
        "KOJO_WORKTREE_ACQUISITION_KEY",
        "KOJO_WORKTREE_INSPECTION_FILE",
        "KOJO_WORKTREE_PROVIDER_IDENTITY",
      ]);
    }).pipe(Effect.provide(layerFor())),
  );
...
describe("sandboxes nesting as branches of the graph", () => {
  it.effect("opens the inner one inside the outer, and closes it first", () =>
    Effect.gen(function* () {
      ...
      const result = Option.getOrThrow(
        yield* serviceFreeWorkflowEffect(nested.definition.poll(executionId)),
      );
      const exit = (result as Workflow.Complete<string, never>).exit;
...
describe("a sandbox scope put inside a phase instead of around one", () => {
  it.effect("never suspends, rebuilds ten times over, and dies", () =>
    Effect.gen(function* () {
      ...
      expect(yield* serviceFreeWorkflowEffect(misplaced.definition.poll(executionId))).toEqual(
        Option.none(),
      );
      ...
      const result = Option.getOrThrow(
        yield* serviceFreeWorkflowEffect(misplaced.definition.poll(executionId)),
      );
      const exit = (result as Workflow.Complete<string, never>).exit;
