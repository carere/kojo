 ...
 *  - **A file the agent *creates* under a hidden directory is not covered.** `.kojo/evil.ts` has no
 *    index entry, so `git add --all` stages it and the merge lands it. Measured. `withPermissions`
 *    and `factoryOwnPaths` are what catch that, which is why rollback is a second line of defence
