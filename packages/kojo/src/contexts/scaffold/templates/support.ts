...
export const readme = (choices: FactoryChoices, starter: Starter): string => {
  ...
  return [
    ...
    "",
    "### Unattended runs do not call an agent unless you say so",
    "",
    "An agent call costs money, and the process that makes it is often one nobody is watching — a CI",
    "job, a cron, an agent driving your shell. So the **invoker** asks one question before it starts",
    "a process, and it asks it every time:",
    "",
    "`KOJO_AGENT_SPEND` is the answer, and it has four:",
    "",
    "- **unset, with a terminal attached** — the call is made. This is you, at a keyboard.",
    "- **unset, with no terminal** — refused, before a process exists. Nothing is spawned and",
    "  nothing is spent.",
    "- **`allow`** — the call is made, wherever it runs.",
    "- **`stand-in:<absolute path>`** — the call is made only if the agent's binary really resolves",
    "  to that file. For rehearsing a workflow against a script.",
    "",
    "So a factory that runs unattended has to say `KOJO_AGENT_SPEND=allow` once, in the environment",
    "of whatever runs it. Until it does, the run stops at its first agent phase with a refusal that",
    "names the agent, the model and the run — never with a bill.",
    "",
    "`stand-in:` is checked rather than believed: Kojo resolves the binary itself and refuses",
    "anything that is not the file you named. Putting a script in front of the real one on `PATH` is",
    "**not** the same claim, and the difference is not academic — it is how the Kojo build itself",
    "spent two agent calls nobody had authorised.",
    "",
    "`kojo doctor` prints which of the four rows you are in, so you can see it before you wonder.",
    "",
    "## What approval does",
    "",
