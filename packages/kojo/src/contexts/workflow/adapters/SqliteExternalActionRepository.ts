      ...
      try: () =>
        this.#database
          .query<
            ...
            [string, string]
          >(
            `SELECT DISTINCT claim.run_id, claim.runner_instance_id, claim.generation,
                    claim.revision_id
               FROM workflow_claims claim
               JOIN workflow_runs run ON run.run_id = claim.run_id
               JOIN workflow_external_actions action ON action.run_id = claim.run_id
              WHERE run.project_id = ? AND claim.runner_instance_id = ?
                AND action.state IN ('unresolved', 'repetition-safe')
              ORDER BY claim.run_id`,
          )
          .all(projectId, runnerInstanceId)
          .map((row) => ({
            runId: row.run_id,
