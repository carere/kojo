...
it.effect("keeps cancellation intent distinct until the executing authority has stopped", () =>
  Effect.gen(function* () {
    ...
    yield* repository.recordCleanupFault(
      [admission.run.runId],
      "the process group still owns a Resource",
    );
    expect(yield* repository.read(admission.run.runId)).toMatchObject({
      cancellation: { state: "requested" },
      cleanup: { state: "fault", detail: "the process group still owns a Resource" },
    });
    expect(
      Result.isFailure(
        yield* Effect.result(
          repository.completeRun(authority, "succeeded", new Date(4).toISOString()),
        ),
      ),
    ).toBe(true);
    ...
    const staleWrite = yield* Effect.result(
      repository.completeRun(authority, "succeeded", new Date(6).toISOString()),
    );
