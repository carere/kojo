...
describe("a hand-written loop", () => {
  it.effect("runs every round against one stale verdict, and never suspends again", () =>
    Effect.gen(function* () {
      const executionId = yield* naive.definition.execute(
        { subject: "fixed", limit: 5 },
        { discard: true },
      );
      yield* settle;
      expect(yield* statusOf(naive.definition.poll(executionId))).toBe("Suspended");
      ...
      expect(yield* statusOf(naive.definition.poll(executionId))).toBe("Complete");
...
describe("the reviewed loop", () => {
  it.effect("suspends on every asking, and leaves one gate record per round", () =>
    Effect.gen(function* () {
      const executionId = yield* looped.definition.execute(
        { subject: "distinct", limit: 5 },
        { discard: true },
      );
      yield* settle;

      expect(yield* statusOf(looped.definition.poll(executionId))).toBe("Suspended");
      expect(yield* requested).toHaveLength(1);
      ...
      expect(yield* statusOf(looped.definition.poll(executionId))).toBe("Complete");
      const result = Option.getOrThrow(yield* looped.definition.poll(executionId));
  ...
  it.effect("lets a failure from the revision travel out untouched", () =>
    Effect.gen(function* () {
      const executionId = yield* brittle.definition.execute(
        { subject: "broken" },
        { discard: true },
      );
      yield* settle;

      const result = Option.getOrThrow(yield* brittle.definition.poll(executionId));
