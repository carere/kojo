...
import { Cause, Duration, Effect, type Exit, Layer, Option, Schema } from "effect";
...
const layerFor = (engine: Layer.Layer<WorkflowEngine.WorkflowEngine>) =>
  Layer.mergeAll(factory.layer, nested.layer, laned.layer).pipe(
    Layer.provideMerge(
      Layer.mergeAll(
        InMemoryTracer.layer,
        InMemoryGate.layer().pipe(Layer.provideMerge(engine)),
...
describe("a run that fails after changing something outside itself", () => {
  it.effect("returns the ticket to the status it really found, and reports the failure", () =>
    Effect.gen(function* () {
      ...
      expect(put).toEqual(["ticket -> ready (NotAccepted)"]);
      ...
      expect(board.comments[0]).toContain(`The branch ${runBranch(runId)} is preserved.`);
    }).pipe(Effect.provide(layerFor(InMemoryClusterEngine.layer))),
  );
...
describe("the same run on the in-memory engine", () => {
  /**
   * **A divergence between the two engines, written down rather than left to be found.**
   *
   * `InMemoryEngine` hands the previous instance's scope to the new instance on every resume
   * (`WorkflowEngine.ts:618-622`), so one run has **one** instance scope for its whole life and each
   * replay's registration piles into it. Closing it at the end runs all of them. The engine Kojo
   * ships builds a fresh instance — and a fresh scope — per execution, and only the last one closes,
   * so it fires once. Same body, same undo, two answers.
   *
   * The consequence for an author is worth stating: this is a property of the double, not of the
   * factory. Nothing needs fixing in the workflow above.
   */
  it.effect("compensates once per execution of the body rather than once per run", () =>
    Effect.gen(function* () {
      const runId = yield* start(factory.definition, { subject: "double" });
      yield* settleThenAdvance(Duration.zero);
      yield* answer("reject", "no");

      expect(yield* status(factory.definition, runId)).toBe("failed");
      // Two executions of the body — the first one and the replay — and therefore two of each.
      expect(put).toEqual(["ticket -> ready (NotAccepted)", "ticket -> ready (NotAccepted)"]);
      expect(ended).toEqual(["failed", "failed"]);
      // The end state is still right, because putting a status back is the same answer twice. An
      // undo that is not like that — one that posts, or charges, or sends — must not be graded here.
      expect(board.status).toBe("ready");
    }).pipe(Effect.provide(layerFor(InMemoryEngine.layer))),
  );
});
