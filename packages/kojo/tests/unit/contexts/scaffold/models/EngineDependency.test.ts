...
const installedEngine: ResolvedPackage = {
  name: "kojo",
  version: "1.4.0",
  directory: "/repo/node_modules/kojo",
};
...
const checkedOutEngine: ResolvedPackage = {
  name: "kojo",
  version: "0.0.0",
  directory: "/home/somebody/kojo/packages/kojo",
};
...
describe("what a stamped repository has to declare", () => {
  it("names the version, when the engine doing the stamping was installed", () => {
    const declared = dependencyFor({ engine: installedEngine, effect: installedEffect });

    expect(declared.reach).toBe("published");
    expect(declared.kojo.specifier).toBe("1.4.0");
  ...
  it("points both entries at this machine, when the engine is a checkout with no version to name", () => {
    const declared = dependencyFor({ engine: checkedOutEngine, effect: checkedOutEffect });

    expect(declared.reach).toBe("linked");
    expect(declared.kojo.specifier).toBe("file:/home/somebody/kojo/packages/kojo");
  ...
  it("keeps the version beside the specifier, because a diagnosis names versions", () => {
    ...
    expect(declared.kojo.version).toBe("0.0.0");
    expect(declarations(declared).map((entry) => entry.name)).toEqual(["kojo", "effect"]);
  });
});
