...
it.effect("never reads a replacement symlink after the Artifact path was observed", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const projectPath = yield* Effect.acquireRelease(
        Effect.promise(() => mkdtemp(join(tmpdir(), "kojo-artifact-store-race-"))),
      ...
      yield* Effect.promise(() =>
        mkdir(join(projectPath, ".kojo", "artifacts", runId), { recursive: true }),
      );
      yield* Effect.promise(() => writeFile(artifactPath, "safe Artifact content"));
      yield* Effect.promise(() => writeFile(outsidePath, "outside Artifact content"));
      ...
      // the outside content.
      race.target = yield* Effect.promise(() => realpath(artifactPath));
      race.swap = () =>
        rename(artifactPath, safePath).then(() => symlink(outsidePath, artifactPath));
      ...
      if (!result.ok) {
        expect(["artifact-content-changed", "artifact-content-unsafe"]).toContain(
          result.failure._tag,
        );
      }
      expect(race.swap).toBeUndefined();
      expect((yield* Effect.promise(() => lstat(artifactPath))).isSymbolicLink()).toBe(true);
    }),
  ).pipe(
    Effect.ensuring(
      Effect.sync(() => {
