...
const loadKojoConfiguration = async (path: string) => {
  ...
  if (!built.success || built.outputs.length !== 1) throw new Error("invalid configuration");
...
export const initializeProject = async (path: string): Promise<ProjectSnapshot> => {
  ...
  if (configuration !== undefined) {
    try {
      await loadKojoConfiguration(configurationPath);
    } catch {
      throw new ProjectInitializationError(
        `${configurationPath} Kojo Configuration is invalid; no files were changed.`,
      );
    }
  }
